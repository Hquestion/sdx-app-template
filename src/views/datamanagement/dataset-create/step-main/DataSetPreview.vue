...
import DataSetPreviewActionsMixin from './mixins/DataSetPreviewActionsMixin';
import CusElTableColumn from '../../../datas/rely/table/src/table-column';
import CusElTable from '../../../datas/rely/table';

declare type ColumnItem = { label: string };
declare type Column = { column: ColumnItem, $index: number };
...
export default {
    ...
    methods: {
        ...
        handlePick(option: Array<string>) {
            // TODO: 暂时写死，需要优化
        ...
        handleRenderHeader(h: any, { column, $index }: Column) {
            let iconClass = '';
        ...
        renderSubHeader(h: any, { column }: Column) {
            let field = this.columns.find(
                item => item.fieldName === column.label
            );
            if (!field) return <span />;
        ...
        loadMore(direction: string, scrollDistance: number, isReachBottom: boolean): void {
        ...
        handleActiveItemChange(value: any) {
            this.$nextTick(() => {
                this.updatePopper();
            });
    ...
    watch: {
        actionHistory(val: any) {
            this.schemaActions = val;
        },
        columns: {
            handler(val: Array<any>) {
                if (val && val.length > 0) {
                    this.$nextTick(() => {
                        this.calcViewPortMaxCount();
